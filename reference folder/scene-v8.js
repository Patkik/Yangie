...
export default class KiroUnifiedSceneV5 {
    ...
    buildKiro() {
        ...
        // Procedural Outline builder for 100% realistic hand-drawn anime look [cite: 300]
        const addCelOutline = (mesh, thickness = 0.04) => {
            const outlineMat = new THREE.MeshBasicMaterial({
                color: 0x11111b, // Charcoal dark ink contour outline [cite: 300]
                side: THREE.BackSide
            });
            const outlineMesh = new THREE.Mesh(mesh.geometry.clone(), outlineMat);
            // Dynamic scale tracking so outline matches viscoelastic squishing [cite: 300]
            outlineMesh.scale.copy(mesh.scale).multiplyScalar(1.0 + thickness);
        ...
        // A. Body Mesh with outlines [cite: 300]
        ...
        addCelOutline(this.kiroMesh, 0.035);

        // B. Front White Belly with outlines [cite: 300]
        ...
        // F. Side Arms with outlines [cite: 300]
        ...
        addCelOutline(this.leftArm, 0.05);
        ...
        addCelOutline(this.rightArm, 0.05);

        // G. Sleeping Nightcap with outlines [cite: 300]
        ...
        addCelOutline(starPompom, 0.06);

        this.kiroGroup.add(this.nightcap);
        this.nightcap.visible = kiroState.get('isSleeping');

        // H. Floating Golden Crown - Unlocks when Kiro has high wellbeing (> 80) [cite: 300]
        this.crown = new THREE.Group();
        this.crown.position.set(0, 0.85, 0);

        const crownBandGeo = new THREE.CylinderGeometry(0.24, 0.24, 0.07, 16, 1, true);
        const crownMat = new THREE.MeshStandardMaterial({
            color: 0xF9E2AF, // Gold-Glow
            roughness: 0.12,
            metalness: 0.9,
            side: THREE.DoubleSide
        });
        const bandMesh = new THREE.Mesh(crownBandGeo, crownMat);
        this.crown.add(bandMesh);
        addCelOutline(bandMesh, 0.08);
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const spikeGeo = new THREE.ConeGeometry(0.055, 0.2, 4);
            const spike = new THREE.Mesh(spikeGeo, crownMat);
            spike.position.set(px, 0.09, pz);
            spike.rotation.y = -angle;
            spike.rotation.z = -0.16;
            this.crown.add(spike);
            addCelOutline(spike, 0.08);

            const beadGeo = new THREE.SphereGeometry(0.024, 8, 8);
            const beadMat = new THREE.MeshBasicMaterial({ color: 0x94E2D5 }); // Emerald jewel [cite: 300]
            const bead = new THREE.Mesh(beadGeo, beadMat);
            bead.position.set(px * 1.15, 0.19, pz * 1.15);
            this.crown.add(bead);
            addCelOutline(bead, 0.09);
        }

        this.kiroGroup.add(this.crown);
        this.crown.visible = kiroState.get('wellbeing') > 80;

        // I. Golden Well-Rested Aura Sphere
    ...
    bindState() {
        ...
        kiroState.on('change:wellbeing', (data) => {
            ...
            // Dynamically scale/toggle Kiro's hand-drawn crown on wellbeing milestones [cite: 300]
            if (this.crown) {
                const isThriving = data.newValue > 80;
                this.crown.visible = isThriving;
                if (isThriving) {
                    gsap.fromTo(this.crown.scale, { x: 0, y: 0, z: 0 }, { x: 1, y: 1, z: 1, duration: 0.8, ease: 'back.out(1.8)' });
                }
            }
        });
