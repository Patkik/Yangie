/**
 * 🛰️ Kiro's Cosmic Haven — Master Skyrim-Inspired Anime Shader Pipeline (V7.0)
...
// Simplex 3D Noise generator for organic gas and hand-drawn canvas textures
...
// Fractional Brownian Motion (fBm) for soft watercolor washes
float fbm(vec3 p) {
    float value = 0.0;
    float amplitude = 0.5;
    float frequency = 1.0;
...
// =============================================================================
export function createPlanetShaderMaterial(planetColorHex, lightDirVector) {
    ...
    return new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                float spiralAngle = angle + (radius * 7.5) - (uTime * 0.15);
                
                // Swirling coordinates
                vec3 spiralCoord1 = vec3(cos(spiralAngle) * radius * 3.5, sin(spiralAngle) * radius * 3.5, uTime * 0.04);
                vec3 spiralCoord2 = vec3(cos(spiralAngle + 3.1415) * radius * 2.5, sin(spiralAngle + 3.1415) * radius * 2.5, -uTime * 0.03);
                ...
                float auroraY = uv.y + sin(uv.x * 3.5 + uTime * 0.6) * 0.12 - 0.55;
                float auroraWave = exp(-auroraY * auroraY * 18.0);
                float auroraNoise = fbm(vec3(uv.x * 2.0, uv.y, uTime * 0.08)) * 0.5 + 0.5;
                ...
                vec3 primaryColor = mix(uColorPrimary, uColorGlow, noise1 * 0.4);
                vec3 secondaryColor = mix(uColorSecondary, uColorBackdrop, noise2 * 0.5);
...
// Ensure full dual compatibility with earlier scene files [cite: 300]
...
// =============================================================================
export function createAnimeStarfieldShaderMaterial() {
    return new THREE.ShaderMaterial({
        transparent: true,
        depthWrite: false,
        blending: THREE.AdditiveBlending,
        uniforms: {
            uTime: { value: 0 },
            uBaseSize: { value: 55.0 } // slightly larger for gorgeous hand-drawn shapes
        },
        vertexShader: `
            uniform float uTime;
            ...
            void main() {
                ...
                float twinkle = 0.35 + 0.65 * sin(uTime * 3.2 + aPhase) * cos(uTime * 1.5 + aPhase * 0.3);
                vAlpha = twinkle;

                // Swirl positions dynamically along the polar vortex stream
                vec3 pos = position;
                float radius = length(pos.xy);
                float swirlAngle = sin(uTime * 0.08 - radius * 0.12) * 0.42 * (1.0 / (radius + 0.5));
                ...
                // Alternate star colors between Mint, Pink, and Gold
                float colorGroup = mod(aPhase, 3.0);
                if (colorGroup < 1.0) {
                    vColor = vec3(0.31, 0.79, 0.69); // #4EC9B0 Mint-Teal
                } else if (colorGroup < 2.0) {
                    vColor = vec3(0.96, 0.71, 0.91); // #F5C2E7 Pastel Pink
                } else {
                    vColor = vec3(0.98, 0.89, 0.69); // #F9E2AF Gold-Glow
                }

                vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
                gl_PointSize = uBaseSize * aSizeMultiplier * (30.0 / -mvPosition.z) * (twinkle * 0.25 + 0.75);
    ...
    return new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                float waveOffset = sin(uTime * 9.5 - aIndex * 5.0) * 0.22 * aIndex;
                pos.y += waveOffset;

                vOpacity = exp(-aIndex * 1.6);

                vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
                gl_PointSize = 32.0 * (1.0 - aIndex * 0.5) * (20.0 / -mvPosition.z);
                gl_Position = projectionMatrix * mvPosition;
            }
        `,
        fragmentShader: `
...
// =============================================================================
export function createAnimeAsteroidShaderMaterial(lightDirVector) {
    return new THREE.ShaderMaterial({
        uniforms: {
            uTime: { value: 0 },
            uLightDir: { value: lightDirVector.clone().normalize() },
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                // 1. Hand-Drawn Outline ink stroke using Rim-Normal detection [cite: 147]
                float edgeRim = dot(normal, viewDir);
                float edgeStroke = step(0.24, edgeRim); // Crisp black ink outline limit [cite: 147]

                // 2. High-fidelity watercolor brush texture overlay on the rock face
                vec3 paintCoord = vec3(vUv.x * 25.0, vUv.y * 25.0, uTime * 0.05);
                ...
                float auraTwinkle = 0.65 + 0.35 * sin(uTime * 4.0 + normal.x * 10.0);
