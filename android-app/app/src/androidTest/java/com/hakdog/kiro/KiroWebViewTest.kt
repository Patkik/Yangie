...
import com.starlight.sanctuary.MainActivity
...
class KiroWebViewTest {
    ...
    fun onboarding_personaSelectionFlow_transitionsToDashboard() {
        ...
        // 2. Ensure that selection portal cards exist in the DOM
        onWebView()
            .withElement(findElement(Locator.CLASS_NAME, "persona-portals-grid"))
            .check(webMatches(androidx.test.espresso.web.webdriver.DriverAtoms.getText(), containsString("Patrick")))

        // 3. Simulate tapping Patrick's glassmorphic companion card portal
        onWebView()
            .withElement(findElement(Locator.CSS_SELECTOR, ".portal-card.patrick"))
            .perform(webClick())

        // 4. Tap the explicit glowing "Choose" button to trigger transition arrival sequence
        onWebView()
            .withElement(findElement(Locator.CSS_SELECTOR, ".portal-card.patrick .portal-choose-btn"))
            .perform(webClick())
